...
//!
//! Each possible integer value is assigned a unique [`String`] representation, allowing for lossless conversion between [`String`]s and [`u32`] values. Each additional character in a [`String`] representation can be thought of as another digit in a [Base62] system. The [Base62] system is similar to the [hexadecimal](https://en.wikipedia.org/wiki/Hexadecimal) system except that instead of only 16 digits, 62 digits are used. Here is a list of all 62 digits used by this crate, in order from least to greatest value:
//! ```no_run
//! # /*
...
pub mod encoding {
    ...
    /// # Arguments
    /// This function takes two arguments. The first, `value`, is a [`u32`] indicating what numerical value the [`String`] representation produced by the function should represent in [Base62].
    ...
    /// [Base62]: https://en.wikipedia.org/wiki/Base62
    /// # Output
    /// Returns a [`Result<String,EncodingError>`] that may store either the [`String`] generated by this function, which stores a [Base62] representation of the `value` argument, or an [`EncodingError`]. If an [`EncodingError`] is returned, please record
